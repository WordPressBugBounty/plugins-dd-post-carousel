/**
 * Owl Carousel Admin JS
 *
 * @author Howard Ehrenberg
 * @package Custom Post Carousels
 */
	...
	function ( $ ) {
		...
		$( '#dd_shortcode_copy' ).on(
			...
			async function () {
				...
				await navigator.clipboard.writeText( aux.value ); // Copy the highlighted text using Clipboard API.
				...
				setTimeout(
					function () {
						document.getElementById( 'dd_owl_shortcode' ).innerHTML = shortcode;
					},
					1000
				);
			}
		...
		if ( taxOptions !== null && 0 === taxCounter) {
			$tax_options.trigger( 'change' );
			taxCounter = 1;
		}
